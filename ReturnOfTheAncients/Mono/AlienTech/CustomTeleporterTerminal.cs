...
namespace RotA.Mono.AlienTech
{
    public class CustomTeleporterTerminal : HandTarget, IHandTarget
    {
        ...
        public void OnHandClick(GUIHand hand)
        {
            if (!Unlocked)
            {
                if (TryEnterItem(out GameObject pickupable))
                {
        ...
        private bool TryEnterItem(out GameObject pickupableObject)
        {
            foreach (TechType tt in acceptedTechTypes)
            {
                Pickupable pickupable = Inventory.main.container.RemoveItem(tt);
                if (pickupable != null)
                {
                    pickupableObject = pickupable.gameObject;
                    return true;
                }
            }
